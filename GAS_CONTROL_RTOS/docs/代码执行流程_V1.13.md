...
> 适用版本：V1.13（2026-09-03）
>
> 本文按照“谁调用当前函数 → 当前函数配置什么 → 当前函数继续调用谁 → 如何处理调用结果 → 当前函数返回到哪里 → 调用者下一步执行什么”的顺序说明程序执行过程。
>
> 气瓶七状态、自动换瓶条件、串口屏控件、日志格式和通信地址等业务定义，请配合 [程序运行流程说明_V1.13.md](程序运行流程说明_V1.13.md) 阅读。
...
| 3 | `F_ValveControl_Task()`推进12V吸合转5V保持 | 紧急全关重试 |
| 4 | `emergency_close_pending`有效时调用 `A_GasControl_ForceAllValvesOff()` | HMI协议解析 |
| 5 | `A_Hmi_Task()`解析按钮、文本、RTC并周期请求RTC | 日志条件文本处理 |
| 6 | `A_HmiLog_InputTask()`处理日期和时间输入 | 参数文本处理 |
| 7 | `A_HmiConfig_InputTask()`处理参数页文本输入 | HMI按钮分发 |
| 8 | `A_GasControl_ProcessHmiButton()`执行一条按钮事件 | HMI参数保存请求 |
| 9 | `A_GasControl_ProcessHmiConfig()`校验、保存并应用参数 | 日志清除请求 |
| 10 | `A_GasControl_ProcessHmiLogClear()`推进日志物理清除 | 人工阀门超时处理 |
| 11 | `A_GasControl_ManualValveTask()`关闭到时阀门和停用瓶阀门 | 六瓶状态更新 |
| 12 | `A_GasControl_UpdateCylinderStates()`维护七种气瓶状态 | 自动切瓶状态机 |
| 13 | `A_GasControl_SwitchTask()`推进初始选瓶或自动换瓶 | 总测试阀联动 |
| 14 | `A_GasControl_TotalTestTask()`推进总阀预开和分路延时开启 | 安全不变量检查 |
| 15 | `A_GasControl_CheckOutputInvariant()`检查十九路阀门安全关系 | 日志记录 |
| 16 | 条件调用 `A_GasLog_Task()`写事件或常规日志 | 参数页刷新 |
| 17 | `A_HmiConfig_Task()`分时发送一项参数页内容 | 日志查询任务 |
| 18 | `A_HmiLog_Task()`推进索引、翻页和逐行发送，显式接收RTC有效标志 | 当前外部通信分支 |
| 19A | CAN模式：更新日志信息，调用 `A_Can_Task()`并显式传入当前参数，再检查CAN故障 | CAN控制请求 |
| 19B | RS485模式：更新日志信息、刷新寄存器，调用 `A_Modbus_Task()`并显式传入当前参数，再检查故障 | CAN控制函数快速返回 |
| 20 | `A_GasControl_ProcessCanControl()`；非CAN模式或无请求时立即返回 | 外部参数请求 |
| 21 | `A_GasControl_ProcessExternalConfig()` | 外部日志读取请求 |
| 22 | `A_GasControl_ProcessExternalLogRead()` | HMI监控刷新判断 |
...
`A_GasControl_Task()`完成HMI刷新判断后直接返回，随后：
