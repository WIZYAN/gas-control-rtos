...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.11 | 2026-08-29 | 当前版本 | 测试阀最长开启时间由5～60秒调整为5～60分钟，上电及恢复默认采用10分钟；Screen4控件90单位改为`min`并由屏端与MCU双重限制；运行时继续使用毫秒截止时间，EEPROM参数记录升级为V4并以分钟保存，V3旧5～60秒设置按相同界面数值迁移为5～60分钟，V2记录补入10分钟默认值；修复日志日期范围连续输入时文本事件被丢弃的问题，开始/结束日期与查询可连续上报，查询快照稳定按完整闭区间筛选 |
...
hmi-screen/GasControl_HMI_V1.11/         八页VisualTFT正式画面，包含日志条件页（气瓶/状态下拉菜单）、分类分页结果页、参数确认和日志清除确认画面
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.11`与
...
`Debug/`。V1.11验证结果为：`text=61060`字节、`data=72`字节、`bss=12424`字节，
总计`73556`字节。
