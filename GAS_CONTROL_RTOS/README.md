...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.09 | 2026-08-28 | 当前版本 | 在密码参数页增加“清除全部日志”，经独立Screen7二次确认后执行；先提交掉电安全的空日志索引，再以64字节为一页物理清除旧管理头和全部日志数据并逐页读回校验；清除过程后台分步运行并显示进度，参数记录和外部通讯模式记录保持不变；CAN和外部Modbus不开放日志清除命令 |
...
hmi-screen/GasControl_HMI_V1.09/         八页VisualTFT正式画面，包含日志条件页、分类分页结果页、参数确认和日志清除确认画面
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.09`与
...
`Debug/`。V1.09验证结果为：`text=60572`字节、`data=72`字节、`bss=12264`字节，
总计`72908`字节；主机测试同时使用MinGW GCC 15.2.0的`-Wall -Wextra -Werror`编译并全部通过，
