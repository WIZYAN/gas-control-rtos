...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.04 | 2026-08-27 | 当前版本 | 事件日志改为每页15条、常规日志改为每页10条；增加“最新页、上一页、下一页”和页码提示；刷新时建立约2KB的单类型RAM索引，满足第1页后立即显示并在后台继续统计；翻页只读取和发送目标页，关闭屏内长列表滑动及滚动条；日志快照变化时保留当前页并提示刷新；串口屏正式工程同步升级为V1.04 |
...
hmi-screen/GasControl_HMI_V1.04/         六页VisualTFT正式画面工程，日志分页且Screen5为独立参数确认画面
...
CAN接口只有在模式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.04`与
...
`Debug/`。V1.04验证结果为：`text=52148`字节、`data=72`字节、
`bss=12128`字节，总计`64348`字节；其中单类型日志索引固定增加2034字节RAM，自编模块没有编译警告。FreeRTOS官方内核源码在工程原有的
