...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.05 | 2026-08-27 | 当前版本 | 1～6号人工排气按钮改为MCU状态同步开关外观；实际排气命令开启时显示“排气中”，定时结束、互锁拒绝或安全关断后恢复“人工排气”；排气中重复点击不重新计时，按钮值0不作为提前关阀命令；增加变化优先回写和58槽周期校正；串口屏正式工程同步升级为V1.05并只保留当前版本工程 |
...
hmi-screen/GasControl_HMI_V1.05/         六页VisualTFT正式画面工程，排气按钮状态同步、日志分页且Screen5为独立参数确认画面
...
CAN接口只有在模式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.05`与
...
串口屏把本地查询分为事件表和常规表。画面2按控件61后，固件保存日志流水号快照，从最新端向旧端扫描原始日志，把事件记录的EEPROM逻辑位置依次保存到RAM索引。索引达到15条后立即把第1页转换为“时间、气瓶、状态变化、压力”四列并发送到控件62，不等待剩余记录统计完成。控件62一屏及最大容量均为15行、单行96字节。

画面3按控件65后采用相同分页索引。控件66是覆盖完整表格的两列控件，列宽为22%时间和78%内容；每条常规日志先发送“时间＋压力”行，再发送“空时间＋状态”行。一屏20个物理行对应10条逻辑日志，控件最大容量也是20行，单行128字节。ID71仅保留编号，不创建第二个表格。
...
可配置排气和测试阀超时、排气/测试阀并行、排气按钮变化优先回写及重复点击不延时、进气/人工阀互锁、停用/重新初始化、地址7配置、控件49总压力显示、HMI按钮
业务映射、RTC读取命令、BCD时间解析、五秒时间失效判断、B1 23图标指定帧、控件72～77连续映射、
使用绿色高亮与低压警告红色高亮、数据记录控件清表、事件15条与常规10条分页索引、第一页优先显示、上一页/下一页/最新页方向、扫描进度、日志快照变化提示、事件四列格式化、常规两列双行对齐顺序、B1 11文本输入、11项页面映射、
...
`Debug/`。V1.05验证结果为：`text=52364`字节、`data=72`字节、
`bss=12136`字节，总计`64572`字节；其中单类型日志索引固定增加2034字节RAM，排气同步上下文增加4字节，自编模块没有编译警告。FreeRTOS官方内核源码在工程原有的
