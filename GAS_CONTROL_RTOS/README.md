<div align="center" style="font-size: 2em; font-weight: bold;">气源控制系统</div>
...
| CAN地址表版本 | `0x0206` |
| Modbus寄存器表版本 | `0x0202` |
| EEPROM运行参数记录版本 | `0x0004`，完整保存13项，测试阀时长以分钟编码 |
...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.14 | 2026-09-04 | 当前版本 | 按新版Renesas嵌入式C项目规则完成失败路径和可调试性加固：内部Modbus在发送前预挂接定长接收，并将SCI1收发、方向恢复和中止失败统一归类为IO错误；任一阀门GPIO或12 V转5 V保持操作失败时立即锁定`platform_ready`并进入紧急全关，全关失败时立即结束本周期并按5 ms重试，各开阀阶段和同批HMI事件之间均设故障检查点，人工排气关阀失败不再提前清除截止时间；HMI按键/下拉菜单改为8项FIFO，未知文本可丢弃计数，固定按钮帧严格校验长度，SCI9错误批次丢弃残帧及待执行输入并立即使RTC无效；EEPROM软件IIC区分BUS、NACK和写完成TIMEOUT，检查GPIO、START/STOP实际释放、读写字节和WP恢复，锁存写保护故障，自检采用必然变化的图样且恢复后读回验证；外部Modbus锁存方向/匹配电阻及包含BREAK的UART错误并完善初始化回滚，外部通讯模式切换逐项检查关闭、恢复和回写结果；移除无读取者的`scheduler_started`。同步更新参数映射、流程文档和用户手册版本说明。正常时序、RTOS/FSP配置、CAN/Modbus协议版本、EEPROM参数/日志格式及大彩工程均保持不变，HMI工程仍为V1.12。 |
| V1.13 | 2026-09-01至03日 | 历史版本 | 统一人工阀门的安全门槛：当硬件平台未就绪时，串口屏和业务接口均拒绝排气阀、测试阀开启请求；关阀请求仍始终允许，CAN原有硬件未就绪详细错误码保持不变；紧急全关改为检查全部25路GPIO写入结果，任一路失败时保留阀门命令与硬件镜像、锁定平台未就绪并每5 ms自动重试，全部成功后才清零软件状态；完成可读性优先重构，删除只转发调用的运行时和内部Modbus硬件层，硬件与协议上下文改为同级显式传递，移除CAN/Modbus生效参数副本，展开日志起止时间字段并分离RTOS任务资源与气源业务实例；硬件行为、协议地址和EEPROM格式保持不变；增加相应主机回归测试 |
| V1.12 | 2026-09-01 | 历史版本 | 完善正式发布资料：删除已失效且指向不存在V1.08工程的串口屏资源工具；新增用户操作手册Markdown可编辑源稿和PDF一键生成脚本，生成器直接读取当前大彩画面与TFT控件定义；修正运行流程说明中日志控件71的过时描述，明确仅控件62和66使用SDRAM、控件71只保留编号；将PDF固定为Git二进制文件，避免CRLF换行转换；固件业务逻辑、EEPROM格式及通信地址表保持V1.11兼容 |
| V1.11 | 2026-08-29 | 历史版本 | 测试阀最长开启时间由5～60秒调整为5～60分钟，上电及恢复默认采用10分钟；Screen4控件90单位改为`min`并由屏端与MCU双重限制；运行时继续使用毫秒截止时间，EEPROM参数记录升级为V4并以分钟保存，V3旧5～60秒设置按相同界面数值迁移为5～60分钟，V2记录补入10分钟默认值；修复日志日期范围连续输入时文本事件被丢弃的问题，开始/结束日期与查询可连续上报，查询快照稳定按完整闭区间筛选 |
| V1.10 | 2026-08-28 | 历史版本 | 日志条件页气瓶编号和进入状态改为大彩原生下拉菜单控件，并升级为与整机一致的图片文字圆角菜单和整块可点击组合选择框；点选后按0起始索引上传`B1 14`帧，MCU直接保存筛选值；修复日期文本与查询按钮同批到达时查询快照仍使用旧值的问题，合法修改任一日期或时间后自动启用“限定时间”，避免已输入范围被“全部时间”忽略；清除条件仍恢复全部 |
| V1.09 | 2026-08-28 | 历史版本 | 在密码参数页增加“清除全部日志”，经独立Screen7二次确认后执行；先提交掉电安全的空日志索引，再以64字节为一页物理清除旧管理头和全部日志数据并逐页读回校验；清除过程后台分步运行并显示进度，参数记录和外部通讯模式记录保持不变；CAN和外部Modbus不开放日志清除命令 |
| V1.08 | 2026-08-28 | 历史版本 | 删除串口屏、CAN和外部Modbus的人工整机自动/停止切换，系统上电固定自动运行，严重故障仍锁定全阀关闭；通讯切换及Modbus整组参数提交要求六瓶全部停用且十九路阀门关闭；31～48号阀位改为双色图标；增加Screen6日志条件查询；增加VAL_CAL总测试阀内部联动，总阀复用VALP1+，先吸合总阀并满足500 ms共享电源间隔后才打开分路，最后一路测试阀关闭后总阀自动关闭；CAN地址表为0x0206，Modbus寄存器表为0x0202 |
...
```text
GAS_CONTROL_RTOS/
├─ AGENTS.md                     Renesas嵌入式C工程修改、验证和交付规则
├─ PARAMETER_MAP.md              V1.14关键配置、运行状态和硬件时序数据流索引
├─ src/
...
│  ├─ A_Gas_Log.c/.h              常规/事件日志、循环覆盖、双索引恢复、物理清除和CRC校验
...
│  └─ F_Modbus_Poll.c/.h          内部Modbus主站状态机，直接使用气源硬件平台的SCI1接口
├─ modbus/
...
hmi-screen/GasControl_HMI_V1.12/         八页VisualTFT正式画面，包含日志条件页（气瓶/状态下拉菜单）、分类分页结果页、参数确认和日志清除确认画面
tools/can-user/                          CAN用户配置文件
docs/                                    V1.14程序/代码流程说明、用户操作手册PDF及Markdown源稿
tools/generate_user_manual.py            根据源稿和当前大彩工程画面重新生成用户操作手册PDF
```

调用层级保持为：应用层 `A_` 负责业务编排，功能层 `F_` 负责状态机和协议处理，硬件层
`H_` 负责FSP及GPIO操作。气源总控直接持有并初始化唯一的硬件平台实例；内部Modbus主站直接调用
该平台的SCI1接口，不再经过只转发参数的运行时和硬件转接结构。CAN、外部Modbus的协议上下文也
不再反向保存同级硬件指针，任务调用时由拥有者显式传入。`hal_entry.c` 是唯一不设置同名头文件的
自编 C 文件；FSP 固定回调名称是命名规则例外。

关键数据的归属保持单一：`config`是当前生效参数，通讯和串口屏模块只保存待校验的
`pending_config`；`system`是对外可见运行状态；`switch_*`和`total_test_*`是业务状态机的私有过程量。
HMI、CAN和外部Modbus的硬件层与协议层均由各自应用上下文并列持有，不再通过深层结构体
或单纯转发的wrapper访问。

# FreeRTOS运行结构

系统使用FreeRTOS 10.4.3-LTS，关闭动态内存分配，任务控制块和栈由
`A_Gas_Rtos_Context`静态提供，气源业务数据由独立的`A_Gas_Control_Context`静态实例持有。
`hal_entry()`负责将两者传入`A_GasRtos_Start()`；调度器启动后，
...
和各串口发送缓冲不会被多个任务同时修改。串口屏日志查询采用分步调度，每个5 ms调用最多完成一次
存储访问或发送一行数据。这只限制了单周期的操作数量，不代表EEPROM访问本身非阻塞：
`A_Storage → F_At24c256 → H_SoftIic`的软件IIC读写和写完成轮询仍会在当前任务中同步执行到返回。
事件与常规查询从快照最新端向旧端建立单类型RAM索引，满足第1页后立即显示，随后分周期完成总数统计；翻页只读取并发送目标页，每32条原始日志更新一次索引进度。

在100 kHz时，64字节页写的总线传输时间本身就可能超过5 ms，还需叠加EEPROM写完成轮询和软件开销。
这是V1.14保留的实时性技术债，发布前应在实机测量日志追加、分页读取和物理清除路径的最坏执行时间（WCET）。
若实测不满足5 ms周期预算，后续应将存储拆为独立低优先级任务或可中断的分步状态机，本版本未改变该实现。
...
参数（默认5秒）和测试阀最长开启参数（默认10分钟）等安全规则限制。
...
- 测试阀由串口屏开关控制，达到`test_valve_max_time_ms`时强制关闭；默认600000 ms（10分钟），可在密码参数页设置5～60整分钟。
...
画面5为参数逐项独立确认画面，画面6为日志条件查询页，画面7为日志物理清除确认及进度页。
实时监控画面ID定义在 `HMI/A_Hmi.h` 的 `A_HMI_MONITOR_PAGE_ID`，日志查询控件ID定义在
`HMI/A_Hmi_Log.h`，参数与日志清除控件ID定义在 `HMI/A_Hmi_Config.h`。

V1.14将按键和下拉菜单事件改为8项固定FIFO，文本事件继续使用8项固定FIFO；单个5 ms
周期会按到达顺序处理已解析的全部按键，日志和参数模块都不识别的队首文本会被主动丢弃，
不会卡住后续有效输入。`button_event_drop_count`和`text_event_drop_count`分别累计队列满或
上层主动丢弃事件，计数到`UINT32_MAX`后饱和，可用于Watch诊断短时爆发输入。文本和日志行发送会在
计算“载荷长度+11字节帧开销”之前先检查载荷上限，防止`size_t`回绕绕过发送缓冲边界。
主周期在解析前和解析后都检查SCI9错误；错误批次清除硬件缓存、协议残帧、RTC待取值及输入FIFO，
并禁止执行由该批次派生的参数保存、日志清除或阀门操作，直到重新初始化HMI。
...
| 114 | 1 | 实时监控页系统标题 | GBK文本控件，显示“气源控制系统” |
| 115 | 1 | 主菜单系统标题 | Screen0透明背景GBK文本控件，默认显示“气源控制系统”，可直接在VisualTFT中修改 |
...
| 127～130 | 4 | 开始日期、开始时间、结束日期、结束时间 | 画面6数字文本输入；日期固定8位`YYYYMMDD`，时间固定6位`HHMMSS`；任一合法修改自动启用限定时间 |
| 131 | 1 | 全部时间开关 | 1忽略起止时间，0启用闭区间时间筛选 |
| 132 | 1 | 气瓶下拉菜单触发按钮 | 整块组合选择框均可点击，弹出149号菜单；自身不改变筛选条件 |
| 133 | 1 | 气瓶当前条件文本 | 显示“全部气瓶”或1～6号气瓶，由屏端菜单选中值或MCU回写 |
| 134 | 1 | 状态下拉菜单触发按钮 | 整块组合选择框均可点击，弹出150号菜单；自身不改变筛选条件 |
| 135 | 1 | 状态当前条件文本 | 显示“全部状态”或七种气瓶状态，由屏端菜单选中值或MCU回写 |
| 149 | 1 | 气瓶下拉菜单控件 | 图片文字圆角菜单，7项`全部气瓶;1号气瓶;...;6号气瓶;`，索引0～6；只影响事件日志 |
| 150 | 1 | 状态下拉菜单控件 | 图片文字圆角菜单，8项`全部状态;初始化;...;待测试;`，索引0～7；只匹配状态变化后的新状态 |
...
| 142 | 1 | 清除全部日志 | 画面4红色瞬时按钮，进入画面7；参数页密码`243579`已经构成第一道权限保护 |
| 143～145 | 3 | 日志清除标题、当前条数和不可恢复警告 | 画面7 GBK动态文本控件 |
| 146 | 1 | 确认清除 | 画面7瞬时按钮；只在尚未清除时建立一次后台清除任务 |
| 147 | 1 | 返回参数 | 画面7切换回画面4；清除已经开始时返回不会取消后台任务 |
| 148 | 1 | 日志清除状态和进度 | 显示等待确认、清除中百分比、成功或失败 |
...
Screen6气瓶和进入状态下拉菜单使用大彩B1 14帧上传选中项，选中索引从0开始，与筛选值一一对应：

```text
EE B1 14 页面ID(2B) 控件ID(2B) 1A 选中索引(1B) 按下或弹起状态(1B) FF FC FF FF
```
...
有效RTC响应，或SCI9锁存通信错误时，程序把 `valid` 置为false并保留最后一次时间内容供故障诊断；
错误批次中的RTC不会生效，日志不得使用无效时间。
...
| 90 | 测试阀最长开启 | min | 10 | 固定安全范围5～60整分钟，运行时保存为300000～3600000 ms |
...
当前正式版不提供控件94和99，也不允许串口屏、CAN或外部Modbus人工停止或恢复整机。系统上电后固定进入自动控制；阀门冲突、平台异常或切瓶开阀失败仍会进入内部故障锁定并关闭十八路分瓶阀和总测试阀，排障后通过重新上电恢复。CAN公开参数采用运行中单地址原子保存；外部RS485/Modbus整组提交要求六瓶全部停用且十九路阀门全部关闭。参数输入控件80～90采用水平及垂直居中显示。

参数页控件142用于进入日志清除画面7。画面7会再次显示当前日志条数和“永久删除、无法恢复”警告；
只有点击控件146“确认清除”才会建立清除任务，控件147只返回参数页。清除任务开始后不要求停止系统，
人员返回其他画面也不会中止；压力采集、阀门状态机和外部通讯继续运行，但新增日志及本地、外部日志读取
临时暂停。清除进度可在画面7控件148或参数页控件93查看。CAN和外部Modbus没有清除日志命令，避免
远程误删出厂或现场追溯数据。
...
V1.14在启动每笔SCI1请求时先调用`H_GasPlatform_SensorRxStart()`挂接预期长度的接收缓冲，
再切到发送方向并启动`H_GasPlatform_SensorTxStart()`，防止从站在请求末尾后立即响应而丢失开头字节。
SCI1方向GPIO、收发启动/回调、事务中止或恢复接收方向失败时，结果统一为
`MODBUS_POLL_RESULT_IO`，与超时、CRC和协议错误区分后再交给应用层累计传感器通信失败。
...
目标接口初始化成功且模式记录写入、读回校验成功后立即生效。切换时会分别检查原接口关闭、
失败目标接口回滚关闭、原模式EEPROM回写和原接口重新初始化的结果；任一回滚步骤失败都保留
对应存储或通讯报警，不再只修改内存中的模式值。成功模式保存在AT24C256
...
SCI0初始化只有在接收方向和匹配电阻GPIO设置、串口打开及回调注册全部成功后才就绪；
回调注册失败会尝试关闭已打开的SCI0。运行期间的发送方向切换、UART发送启动、发送完成后恢复接收方向，
以及反初始化恢复接收方向的失败都会锁存`uart_error`；`A_Modbus_HasFault()`随后将其反映到
`GAS_ALARM_EXTERNAL_MODBUS`。标准寄存器地址、功能码和响应字节没有变化。
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.14`与
...
13项运行参数由`Gas_Config`保存，并从AT24C256 `0x0000～0x0023`的36字节V4带CRC记录加载。
EEPROM没有有效记录时使用`gas_common.h`中的编译期默认值并尝试写入EEPROM；CAN或外部Modbus仍只
修改原有10项。串口屏密码页显示11项，提交时自动生成回差释放压力并固定压力数据新鲜度，最终仍保存
完整13项记录。V4保持记录长度和字段顺序不变，只把末项测试阀最长开启时间改为5～60分钟整数编码。

旧固件30字节V2和36字节V3记录仍可读取。V2首次启动时保留旧记录的10项值，为新增三项填入
2.0 MPa、5秒和10分钟默认值；V3保留低压警告和排气参数，并把旧测试阀5～60秒设置按相同界面数值
迁移为5～60分钟，例如45秒升级为45分钟。校验通过后记录自动原址写为V4。V2记录迁移前，程序会
先把旧地址`0x0020`的通讯模式迁到`0x0030`，避免36字节参数记录覆盖原CAN/RS485选择。

V1.14将软件IIC关键操作的失败路径连通到`F_At24c256_Result`：SCL/SDA GPIO访问、STOP后的线路实际释放、
读字节采样及ACK/NACK回送、总线恢复和WP恢复任一失败都使本次读写返回失败；单次未应答、写完成轮询超时
和总线故障分别记录为`NACK`、`TIMEOUT`、`BUS`。WP故障另行锁存，自检图样保证不同于原值，恢复原数据后
再次读回确认。上层据此设置存储报警或保留旧参数/日志索引。EEPROM地址划分、页大小和持久化格式未改。
...
| EEPROM地址 | 长度 | 用途 |
|---|---:|---|
| `0x0000～0x0023` | 36字节 | 完整13项运行参数记录，EEPROM结构版本`0x0004` |
...
V1.09的“清除全部日志”执行真正的物理清除，而不是只把有效条数改为0。程序先把一份代数更新、
有效条数为0的日志管理头写入当前非活动副本并读回校验；只有该空索引可靠提交后，才清除旧管理头
所在的64字节页，再从`0x00C0`到`0x7FFF`逐页写入`0xFF`并逐页读回校验。清除范围共510页：
1页旧管理头和509页日志数据。`0x0000～0x003F`参数及通讯配置区和新提交的有效管理头不会被擦除。

清除状态机每个控制周期最多调用一次64字节同步写入或一次同步读回校验，典型完成时间约10～20秒，具体取决于
EEPROM写周期。清除过程中日志数量立即按0处理，新增常规/事件记录以及串口屏、CAN、Modbus日志读取
暂时返回忙；气源自动控制和压力刷新不暂停。若中途掉电，重新上电会优先采用已提交的空管理头，因此
不会把部分清除的旧记录重新公布为有效日志。清除成功后当前半小时时段标记为已经处理，避免立即生成
一条常规日志；之后发生新的真实状态变化或进入下一个半小时时段时恢复正常记录。
“每周期最多一次”不限制单次软件IIC访问的持续时间，上述清除期间“继续运行”表示后续业务会在该同步访问返回后继续，不表示存储操作与控制并行。
...
事件记录在任意气瓶状态与最近已记录状态不同时产生。为限制同步软件IIC页写的发生频率，每次主循环最多写一条事件；
单次写入仍会占用当前控制任务，其32字节格式为：
...
串口屏从主菜单或实时监控页进入画面6日志条件页。默认选择“全部时间、全部气瓶、全部状态”；开始和结束字段分别按`YYYYMMDD`、`HHMMSS`输入，任一字段合法修改后MCU自动切换为“限定时间”并回写131号按钮，查询边界包含开始时刻和结束时刻。V1.11将B1 11文本输入改为8条FIFO：开始日期、结束日期与查询按钮连续到达时，控制任务先按到达顺序提交全部日期/时间字段，再建立不可变查询快照，避免屏幕显示新日期而MCU仍按旧日期（例如误包含8月28日）筛选。开始时间晚于结束时间时不读取EEPROM，并在对应结果页提示“时间范围错误”。V1.10起事件气瓶和进入状态通过下拉菜单点选（控件132/134触发、149/150菜单、133/135显示），选中索引0～6对应全部及1～6号气瓶、0～7对应全部及七种状态；“气瓶”和“目标状态”条件对常规日志无效，因为常规记录是一条同时包含六瓶状态的系统快照。
...
正式发布固件通过PC协议和业务回归测试。测试覆盖：默认CAN模式、29位扩展ID、自定义CRC、float32小端响应、V1.09连续地址表、CAN单参数EEPROM最终应答、参数关系详细错误码、废弃参数提交及整机启停地址、远程排气/测试阀控制及二者并行、测试阀与停用按钮按MCU实际状态同步、六瓶停用且全阀关闭时CAN/RS485切换、模式EEPROM恢复、内部Modbus发前预挂接接收及SCI1 IO错误分类、
...
可配置排气和测试阀超时、测试阀5～60整分钟边界与10分钟默认值、V2/V3到V4参数迁移、排气/测试阀并行、VAL_CAL总测试阀预开启、VALP1+共享吸合间隔、多分路保持及最后一路关闭联动、排气按钮变化优先回写及重复点击不延时、进气/人工阀互锁、停用/重新初始化、地址7配置、控件49总压力显示、HMI按钮
业务映射、串口屏与业务接口的`platform_ready`排气/测试阀开启保护及故障关阀路径、紧急全关25路GPIO结果检查、阀门GPIO/12 V转5 V失败锁定、人工排气关阀失败保留截止时间、失败镜像保留与5 ms周期重试、RTC读取命令、BCD时间解析、五秒时间失效判断、B1 23图标指定帧、控件72～77连续映射、
使用绿色高亮与低压警告红色高亮、数据记录控件清表、事件15条与常规10条分页索引、第一页优先显示、上一页/下一页/最新页方向、扫描进度、日志快照变化提示、日志时间闭区间、事件气瓶和目标状态筛选、无效时间范围拒绝扫描、Screen6文本输入与下拉菜单选择（B1 14帧、索引0～6/0～7映射、触发按钮忽略、越界索引拒绝、弹起重复上传幂等、清除条件恢复全部）、8条B1 11文本输入FIFO、8项按键/菜单FIFO、未知文本丢弃计数、超长发送载荷防回绕拒绝、日期范围与查询按钮连续到达时完整提交快照、事件四列格式化、常规两列双行对齐顺序、11项页面映射、
隐藏回差和压力新鲜度补齐、手动排气3000～65535 ms边界、三项安全范围拒绝、`20/20.0/20.00`压力格式兼容、逐项自动确认、取消不生效、
运行中13项参数EEPROM持久化、低压确认重启策略、废弃整机启停入口、内部严重故障全阀关闭、31～48号阀位关闭原色/开启绿色图标同步，以及V1.09日志空索引提交、510页物理清除、逐页读回校验、参数区保留、清除期间读取忙和清除后重新记录。

已在PC回归用例中增加V1.14的内部Modbus预接收顺序/IO错误、阀门保持切换与关阀失败安全收敛、
同批故障后禁止继续开阀、全关失败周期不进入同步EEPROM、人工排气截止时间保留、HMI连续按键队列、SCI9故障批次RTC与输入失效、固定按钮帧长、
未知文本后续放行及超长发送拒绝断言。这些主机测试使用
MinGW GCC 15.2.0，以`-Wall -Wextra -Wconversion -Wsign-conversion -Werror`编译运行，并另行通过
`-fanalyzer`静态分析；但不模拟真实GPIO、SCI中断时序或EEPROM电气故障。

V1.14已使用工程配套Arm GCC 10.3.1和现有FSP配置执行全量重建及链接，结果为
`text=63124`字节、`data=72`字节、`bss=12352`字节，总计`75548`字节；自编生产源码还通过
`-Wall -Wextra -Wconversion -Wsign-conversion -Werror -fsyntax-only`检查。仍需在真实板卡上回归
阀门失效安全、SCI1紧跟响应、SCI0方向切换、EEPROM总线错误路径，并测量同步存储操作的最坏执行时间。
当前工程编译选项开启`-Wall`和部分定向警告，没有开启`-Wextra`或`-Wconversion`；如将严格警告用于发布门禁，
应对自编源码建立独立检查，不应通过修改FreeRTOS/FSP自动生成或供应商源码来消除第三方警告。

公共API和复杂关键函数使用简洁注释说明职责、输入、输出及失败语义；简单静态函数不为满足形式机械增加
大段模板。硬件特殊时序和安全边界重点说明“为什么”。除`hal_entry.c`和只保存公共数据的
`gas_common.h`外，每个自编`.c`文件均有同名`.h`文件。

# V1.14文档入口与验证边界

| 文档 | 用途 |
|---|---|
| [AGENTS.md](AGENTS.md) | 规定本工程及后续Renesas工程的修改前检查、编码、安全、验证和交付规则。 |
| [PARAMETER_MAP.md](PARAMETER_MAP.md) | 按变量名追踪关键参数的定义、初始化、修改、读取和最终用途，并给出断点与Watch建议。 |
| [程序运行流程说明_V1.14.md](docs/程序运行流程说明_V1.14.md) | 从业务角度说明上电、周期任务、压力、状态、阀门、日志和外部通信流程。 |
| [代码执行流程_V1.14.md](docs/代码执行流程_V1.14.md) | 按caller→当前函数→child calls→返回/失败去向展开函数执行轨迹。 |
| [用户操作手册_V1.14.md](docs/用户操作手册_V1.14.md) | 用户操作手册的可编辑源稿，供后续画面或功能变化时重复生成。 |
| [用户操作手册_V1.14.pdf](docs/用户操作手册_V1.14.pdf) | 面向现场人员的正式操作手册；画面资源仍来自`GasControl_HMI_V1.12`。 |

V1.14影响的生产源码模块为内部Modbus/SCI1平台层、阀门功能与平台层、气源总控、
HMI协议与SCI9故障上报、AT24C256软件IIC、外部Modbus硬件层及RTOS任务资源结构；同步更新
README、参数映射和两份流程文档。用户操作流程和画面未改；V1.14用户手册/PDF仅同步版本说明，
`hmi-screen/GasControl_HMI_V1.12`工程保持不动。

本次修改只加固错误检测、回滚和失效安全路径；正常阀门时序、RTOS任务周期/优先级、FSP配置、
CAN地址表`0x0206`、Modbus寄存器表`0x0202`、外部参数接口`0x0002`、EEPROM参数记录`0x0004`及日志存储格式保持不变。
源码级别已通过符号/调用链复核、PC回归与静态分析，完整Arm目标也已全量编译和链接；实机故障注入及
同步EEPROM路径最坏执行时间仍待板卡验证。
