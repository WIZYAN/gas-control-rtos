...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.13 | 2026-09-01至03日 | 当前版本 | 统一人工阀门的安全门槛：当硬件平台未就绪时，串口屏和业务接口均拒绝排气阀、测试阀开启请求；关阀请求仍始终允许，CAN原有硬件未就绪详细错误码保持不变；紧急全关改为检查全部25路GPIO写入结果，任一路失败时保留阀门命令与硬件镜像、锁定平台未就绪并每5 ms自动重试，全部成功后才清零软件状态；完成可读性优先重构，删除只转发调用的运行时和内部Modbus硬件层，硬件与协议上下文改为同级显式传递，移除CAN/Modbus生效参数副本，展开日志起止时间字段并分离RTOS任务资源与气源业务实例；硬件行为、协议地址和EEPROM格式保持不变；增加相应主机回归测试 |
...
docs/                                    运行流程说明、用户操作手册PDF及其Markdown可编辑源稿
tools/generate_user_manual.py            根据源稿和当前大彩工程画面重新生成用户操作手册PDF
```

调用层级保持为：应用层 `A_` 负责业务编排，功能层 `F_` 负责非阻塞状态机和协议处理，硬件层
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.13`与
...
清除状态机每个控制周期最多完成一次64字节写入或一次读回校验，典型完成时间约10～20秒，具体取决于
...
正式发布固件通过PC协议和业务回归测试。测试覆盖：默认CAN模式、29位扩展ID、自定义CRC、float32小端响应、V1.09连续地址表、CAN单参数EEPROM最终应答、参数关系详细错误码、废弃参数提交及整机启停地址、远程排气/测试阀控制及二者并行、测试阀与停用按钮按MCU实际状态同步、六瓶停用且全阀关闭时CAN/RS485切换、模式EEPROM恢复、
...
业务映射、串口屏与业务接口的`platform_ready`排气/测试阀开启保护及故障关阀路径、紧急全关25路GPIO结果检查、失败镜像保留与5 ms周期重试、RTC读取命令、BCD时间解析、五秒时间失效判断、B1 23图标指定帧、控件72～77连续映射、
使用绿色高亮与低压警告红色高亮、数据记录控件清表、事件15条与常规10条分页索引、第一页优先显示、上一页/下一页/最新页方向、扫描进度、日志快照变化提示、日志时间闭区间、事件气瓶和目标状态筛选、无效时间范围拒绝扫描、Screen6文本输入与下拉菜单选择（B1 14帧、索引0～6/0～7映射、触发按钮忽略、越界索引拒绝、弹起重复上传幂等、清除条件恢复全部）、8条B1 11文本输入FIFO、日期范围与查询按钮连续到达时完整提交快照、事件四列格式化、常规两列双行对齐顺序、11项页面映射、
...
`Debug/`。V1.13验证结果为：`text=61472`字节、`data=72`字节、`bss=12312`字节，
总计`73856`字节。
主机测试使用MinGW GCC 15.2.0的`-Wall -Wextra -Werror`编译并全部通过，
自编模块没有编译警告。FreeRTOS官方内核源码在工程原有的
