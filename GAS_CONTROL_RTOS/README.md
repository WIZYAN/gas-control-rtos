...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.10 | 2026-08-28 | 当前版本 | 日志条件页气瓶编号和进入状态改为大彩原生下拉菜单控件，并升级为与整机一致的图片文字圆角菜单和整块可点击组合选择框；点选后按0起始索引上传`B1 14`帧，MCU直接保存筛选值；修复日期文本与查询按钮同批到达时查询快照仍使用旧值的问题，合法修改任一日期或时间后自动启用“限定时间”，避免已输入范围被“全部时间”忽略；清除条件仍恢复全部 |
...
hmi-screen/GasControl_HMI_V1.10/         八页VisualTFT正式画面，包含日志条件页（气瓶/状态下拉菜单）、分类分页结果页、参数确认和日志清除确认画面
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.10`与
...
串口屏从主菜单或实时监控页进入画面6日志条件页。默认选择“全部时间、全部气瓶、全部状态”；开始和结束字段分别按`YYYYMMDD`、`HHMMSS`输入，任一字段合法修改后MCU自动切换为“限定时间”并回写131号按钮，查询边界包含开始时刻和结束时刻。同批收到输入框失焦文本和查询按钮时先提交文本再建立不可变查询快照。开始时间晚于结束时间时不读取EEPROM，并在对应结果页提示“时间范围错误”。V1.10起事件气瓶和进入状态通过下拉菜单点选（控件132/134触发、149/150菜单、133/135显示），选中索引0～6对应全部及1～6号气瓶、0～7对应全部及七种状态；“气瓶”和“目标状态”条件对常规日志无效，因为常规记录是一条同时包含六瓶状态的系统快照。
...
使用绿色高亮与低压警告红色高亮、数据记录控件清表、事件15条与常规10条分页索引、第一页优先显示、上一页/下一页/最新页方向、扫描进度、日志快照变化提示、日志时间闭区间、事件气瓶和目标状态筛选、无效时间范围拒绝扫描、Screen6文本输入与下拉菜单选择（B1 14帧、索引0～6/0～7映射、触发按钮忽略、越界索引拒绝、弹起重复上传幂等、清除条件恢复全部）、事件四列格式化、常规两列双行对齐顺序、B1 11文本输入、11项页面映射、
...
`Debug/`。V1.10验证结果为：`text=60792`字节、`data=72`字节、`bss=12264`字节，
总计`73128`字节。
