...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.12 | 2026-09-01 | 当前版本 | 完善正式发布资料：删除已失效且指向不存在V1.08工程的串口屏资源工具；新增用户操作手册Markdown可编辑源稿和PDF一键生成脚本，生成器直接读取当前大彩画面与TFT控件定义；修正运行流程说明中日志控件71的过时描述，明确仅控件62和66使用SDRAM、控件71只保留编号；将PDF固定为Git二进制文件，避免CRLF换行转换；固件业务逻辑、EEPROM格式及通信地址表保持V1.11兼容 |
...
CAN接口只有在通讯方式0时运行，因此通过CAN正常读到`0x0117`时其值应为0。软件发布版本`V1.12`与
...
`Debug/`。V1.12验证结果为：`text=61060`字节、`data=72`字节、`bss=12424`字节，
总计`73556`字节。
