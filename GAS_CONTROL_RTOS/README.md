...
| 软件版本 | 版本日期 | 状态 | 版本说明 |
|---|---|---|---|
| V1.13 | 2026-09-01 | 当前版本 | 统一人工阀门的安全门槛：当硬件平台未就绪时，串口屏和业务接口均拒绝排气阀、测试阀开启请求；关阀请求仍始终允许，CAN原有硬件未就绪详细错误码保持不变；紧急全关改为检查全部25路GPIO写入结果，任一路失败时保留阀门命令与硬件镜像、锁定平台未就绪并每5 ms自动重试，全部成功后才清零软件状态；增加相应主机回归测试 |
...
`Debug/`。V1.13验证结果为：`text=61096`字节、`data=72`字节、`bss=12424`字节，
总计`73592`字节。
